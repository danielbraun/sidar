...
from backoffice.sidar_models import Categoriescategory, Categories
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for db_name in LEGACY_DB_NAMES:
            for sidar_category in Categories.objects.using(db_name).all():
                try:
                    category = orm.Category.objects.get(name_he=sidar_category.namehebrew)
                    sidar_parent = Categoriescategory.objects.using(db_name).get(pk=sidar_category.catcode)
                    category.parent = orm.Category.objects.get_or_create(
                        name_he=sidar_parent.namehebrew.strip(),
                        defaults={'name_en': sidar_parent.nameenglish.strip()}
                    )[0]
                    category.save()
