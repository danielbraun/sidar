...
from backoffice.sidar_models import Subjects

from sidar.settings import LEGACY_DB_NAMES

from backoffice.sidar_models import Subjectscategory
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for db_name in LEGACY_DB_NAMES:
            for sidar_subject in Subjects.objects.using(db_name).all():
                try:
                    subject = orm.Subject.objects.get(name_he=sidar_subject.namehebrew)
                    subject.parent = orm.Subject.objects.get_or_create(
                        name_he=Subjectscategory.objects.using(db_name).get(pk=sidar_subject.catcode).name
                    )[0]
                    subject.save()
                except orm.Subject.DoesNotExist:
                    pass
